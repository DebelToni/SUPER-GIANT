    ...
    const resp = await openai.chat.completions.create({
      ...
      messages: [
        ...
        },
        {
          role: 'user',
          content: `Project context:
This project is a python backend for LLM training, inference and visualization. There is a backend server under server/ that manages commands and initializes any training or generation. Under ui/ there is a frontend that visualizes the process and the commands for a user to use the architecture from a remote browser.

The core of the model is written in python using JAX. The model has custom synthetic data-generation, KV caching at inference, an RL workflow and much more in corresponding scripts.
