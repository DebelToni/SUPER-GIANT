...
def load_configs() -> OmegaConf:
    ...
    qa_cfg = getattr(cfg, "qa_finetune", None)
    if qa_cfg is not None:
        if "answers_arrow" in qa_cfg and qa_cfg.answers_arrow is not None:
            resolved = resolve_path(qa_cfg.answers_arrow)
            if resolved is not None:
                qa_cfg.answers_arrow = resolved

        if "checkpoint_dir" in qa_cfg and qa_cfg.checkpoint_dir is not None:
            resolved = resolve_path(qa_cfg.checkpoint_dir)
            if resolved is not None:
                qa_cfg.checkpoint_dir = resolved
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--chat_template", action="store_true",
                        help="Use tokenizer.apply_chat_template with add_generation_prompt=True.")
...
def main():
    ...
    if prompt_ids.shape[0] > context_length:
