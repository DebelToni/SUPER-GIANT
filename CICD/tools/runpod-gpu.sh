...
lookup_pod_id_by_name() {
  local target_name="$1"
  local output=""
  if output=$(runpodctl pod list 2>/dev/null); then
    python3 -c '
import json
import sys
...
if isinstance(data, dict):
    pods = data.get("pods") or data.get("items") or data.get("data") or []
else:
    pods = data
for pod in pods if isinstance(pods, list) else []:
    if isinstance(pod, dict) and pod.get("name") == target:
...
stop_cmd() {
  ...
  if runpodctl pod stop --help >/dev/null 2>&1; then
