...
class GiantGPT(nn.Module):
    ...
    def __init__(
        ...
        *,
        vocab_size: int,
        d_model: int = int(Config.embedding_size),
        n_layers: int = int(Config.num_layers),
        n_heads: int = int(Config.num_heads),
        d_ff: int = int(Config.feed_forward_size),
        dropout: float = float(Config.dropout_rate),
        num_kv: int = int(getattr(Config, "num_kv_heads", 1)),
        rotary_dim: int = int(getattr(Config, "rope_dim", max(2, int(int(Config.embedding_size) // int(Config.num_heads))))),
