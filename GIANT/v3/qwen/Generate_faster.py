...
def build_model() -> QwenGPT:
    return QwenGPT(
        ...
        dropout_rate=0.0,
        rope_dim=Config.model.embedding_size // Config.model.num_heads,
...
def init_caches(model: QwenGPT, params: dict, batch_size: int = 1):
...
def make_step_fn(model: QwenGPT, temperature: float, top_k: Optional[int]):
    @jax.jit
    def step_fn(
        params: dict,
        cache: dict,
