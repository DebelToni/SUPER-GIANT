...
- acceptance метрики при TiDAR decode;
...
Системата е CLI-first, т.е. управлява се предимно през команди и конфигурационни файлове, без графичен интерфейс.

Това решение е уместно за изследователски проект, защото дава:

- директен контрол на параметрите;
- лесна автоматизация чрез скриптове;
- добра работа в remote shell среди (ssh/tmux).

==== 2.2.4 Избор на изчислителен фреймуърк

JAX е избран заради JIT компилацията и оптимизациите от XLA. На практика това позволява една Python функция да се компилира до силно оптимизиран изпълним граф за конкретни размерности на входа.
...
Optax е използван за optimizer/scheduler логиката, а Orbax - за checkpointing.
...
Потребителят работи чрез entry point скриптове и конфигурационни файлове. Това гарантира, че всеки run е възпроизводим и може да бъде повторен от друг разработчик при същите условия.
...
Критичните метрики, които показват дали даден training run трябва да бъде прекратен или коригиран, се извеждат персистентно на терминалния екран по време на изпълнение в `tmux`. Това осигурява непрекъснат оперативен контрол без необходимост от отделен графичен интерфейс.
