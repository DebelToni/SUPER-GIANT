...
Тази секция представя концептуално и формално как работи Transformer архитектурата, върху която е изграден GIANT. Изложението следва последователност от интуитивно описание на механизма към неговата математическа формализация.

==== 2.5.1 Теоретична отправна точка: Attention Is All You Need

Класическите по-стари езикови модели обработват текста последователно, символ по символ. Това е коректно, но ограничава паралелизма и забавя обучението.
...
Transformer променя това чрез *attention* механизъм. В практичен смисъл attention означава: когато моделът обработва дадена дума, той изчислява кои други думи от контекста са най-важни за нея и им дава по-голяма тежест.

Може да се мисли за attention като "умно претегляне на контекста":

- моделът сравнява текущата позиция с всички предишни позиции;
...
Точно тази идея позволява паралелна обработка на цели блокове от токени и е фундаментална за съвременните LLM системи.

#figure(
  image("../../../images/Attention_diagram_bw_swapped.png", width: 84%),
  caption: [Интуитивна схема на attention механизма],
)
...
- $S$ е softmax функцията за нормализиране на тежестите.
