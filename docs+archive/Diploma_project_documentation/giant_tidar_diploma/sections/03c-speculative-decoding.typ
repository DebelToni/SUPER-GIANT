...
Преди представянето на TiDAR е важно да се въведе идеята за speculative decoding#link(<bib-spec-decoding>)[#super[16]], защото именно тя е базата, върху която стъпва по-късното Anchor-TiDAR разширение.
...
Важно разграничение: класическите speculative decoding методи обикновено използват втори, по-малък draft модел (например по-малка Llama спрямо по-голям verify модел, напр. 3B срещу 70B) или добавят допълнителни архитектурни компоненти върху базовия модел (например допълнителни глави/слоеве, както при EAGLE#link(<bib-eagle>)[#super[17]]).
...
В проекта се използва и FlashAttention#link(<bib-flashattention>)[#super[15]] (cuDNN backend), което намалява IO overhead в attention стъпката и допълнително подпомага този подход.
