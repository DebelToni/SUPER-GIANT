...
През последните години езиковите модели преминаха от изследователски прототипи към масови потребителски продукти. Появата на ChatGPT и последвалите системи (GPT семейство, Claude, Gemini, open-source LLaMA и производни) промени очакванията на потребителите: моделът трябва едновременно да е полезен, контекстно точен и достатъчно бърз за интерактивна работа. Именно това поставя фокуса върху архитектури и decode техники, които подобряват скоростта без рязък компромис в качеството.
...
Езиковият модел е софтуерен модел, който предсказва вероятността за следващa дума в текстова последователност. На практика това позволява генериране на нов текст, дописване на изречения и отговори в чат сценарии.
...
Суперскриптните индекси насочват към материал в разделa "Използвана литература".
...
- `optimizer`#link(<bib-adam>)[#super[6]] - алгоритъм за обновяване на параметрите по посоката на градиент ( AdamW #link(<bib-adamw>)[#super[14]]);
...
- `LayerNorm`#link(<bib-layernorm>)[#super[13]] - нормализация на активации чрез средна стойност и стандартно отклонение;
...
vLLM#link(<bib-vllm>)[#super[2]] и TensorRT-LLM са системи за ефективно изпълнение на вече обучени езикови модели. Те намаляват закъснението и повишават пропускателната способност чрез оптимизирано управление на KV кеша и GPU изпълнението. В проекта тези практики се използват като инженерен ориентир при оценката на скоростта и при избора на decode политики.
