...
RMS_EPS = float(getattr(MODEL_CFG, "rms_norm_eps", 1e-5))
...
class NativeJaxSelfAttention(nn.Module):
    ...
    def setup(self):
        ...
        rot_dim = self.rotary_dim or self.head_dim
        assert rot_dim % 2 == 0, "rotary_dim must be even"
        ...
        # stash as static field
        object.__setattr__(self, "rotary_dim", rot_dim)
        self._rope_sin, self._rope_cos = _build_rope_cache(
            MODEL_CFG.context_length, self.rotary_dim, self.rope_theta, self.dtype
        )
...
class TinyTransformerBlock(nn.Module):
    ...
    def __call__(self, x, *, deterministic: bool, use_kv_cache: bool = False, cur_index: Optional[int] = None):
        def _block(module: "TinyTransformerBlock", h: jnp.ndarray) -> jnp.ndarray:
            residual = h
            h_norm = RMSNorm(name="rms1", dtype=self.dtype, epsilon=RMS_EPS)(h)
            h_attn = NativeJaxSelfAttention(
                ...
                dtype=module.dtype,
                rotary_dim=module.d_model // module.n_heads,
                rope_theta=MODEL_CFG.rope_theta,
            )(h_norm, deterministic=deterministic, use_kv_cache=use_kv_cache, cur_index=cur_index)
            h = residual + h_attn

            residual = h
            h_norm = RMSNorm(name="rms2", dtype=self.dtype, epsilon=RMS_EPS)(h)
            h_proj = nn.Dense(
                module.d_ff,
                name="fc1",
                dtype=module.dtype,
            ...
            h_proj = nn.Dropout(rate=module.dropout_rate)(h_proj, deterministic=deterministic)
            return residual + h_proj
